...
### The models also can be adpated depending on your data
...
station$rainday[station$rain > 0.85]= "w" 
station$rainday[station$rain <= 0.85]= "d"
  
  # Then calculate the "lags" of this, which gives you the value for the 
  # previous day. Look at ?lag to do this.
  # You should get lags 1, 2 and 3 which correspond to yesterday, 2 days ago, 
  # and 3 days ago.
  station$lag1 <- lag(station$rainday,1)
  station$lag2 <- lag(station$rainday,2)
  station$lag3 <- lag(station$rainday,3)
...
station$prev1 <- factor(station$prev1)
  ...
  form_zero <- rainday ~ (cos(doy * 1 * 2 * pi/366) + sin(doy * 1 * 2 * pi/366) + 
                            cos(doy * 2 * 2 * pi/366) + sin(doy * 2 * 2 * pi/366)+
                            cos(doy * 3 * 2 * pi/366) + sin(doy* 3 * 2 * pi/366))
fit_zero <- glm(as.formula(form_zero), data = station, family = binomial, na.action = na.exclude)

anova(fit_zero, test = "Chisq")
...
form_two <-  rainday ~ (cos(doy * 1 * 2 * pi/366) + sin(doy * 1 * 2 * pi/366) + 
  ...
  form_two_int <- glm(as.formula(form_two_int), data = station, family = binomial, na.action = na.exclude)
...
form_three_int <- glm(as.formula(form_three_int), data = station, family = binomial, na.action = na.exclude)
...
# AIC Analysis
AIC(fit_zero, fit_one, fit_one_int, fit_two, fit_two_int, fit_three, fit_three_int)

# BIC Analysis
# Do the same for BIC
BIC(fit_zero, fit_one, fit_one_int, fit_two, fit_two_int, fit_three, fit_three_int)
...
# add fitted values i.e. probabilities to data
station$fit_one <- fitted(fit_one)

p1 <- ggplot(station, aes(x = doy, y = fit_one, colour = prev1)) + 
  geom_line() + 
  ylab("Probability") + 
  xlab("Day of Year")
p1
...
# add fitted values i.e. probabilities to data
station$fit_one_int <- fitted(fit_one_int)

p1_int <- ggplot(station, aes(x = doy, y = fit_one_int, colour = prev1)) + 
  geom_line() + 
  ylab("Probability") + 
  xlab("Day of Year")
p1_int
...
# add fitted values i.e. probabilities to data
station$fit_two <- fitted(fit_two)
  
  p2 <- ggplot(station, aes(x = doy, y = fit_two, colour = prev2)) + 
    geom_line() + 
    ylab("Probability") + 
    xlab("Day of Year")
  p2
...
## Plotting third-order MC model
  station$fit_three <- fitted(fit_three)
  
  p3 <- ggplot(station, aes(x = doy, y = fit_two, colour = prev3)) + 
    geom_line() + 
    ylab("Probability") + 
    xlab("Day of Year")
  p3
